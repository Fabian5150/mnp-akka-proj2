...
public class FormatterCont extends AbstractBehavior<FormatterCont.Message> {
    ...
    public static Behavior<Message> create(String first_value, String operation, ActorRef<Message> cust, LeftOrRight pos) {
    ...
    private Behavior<FormatterCont.Message> onCalc(FormatterCont.Calc msg)
    {
        ...
        else
        {
            if(msg.pos==LeftOrRight.Right)//It is intended that the last arrived value is the right one
                this.cust.tell(new Calc(MessageFormat.format("({0}{1}{2})", firstString, this.operation, msg.val), this.position));
            else
                this.cust.tell(new Calc (MessageFormat.format("({0}{1}{2})", msg.val, this.operation, firstString), this.position));
