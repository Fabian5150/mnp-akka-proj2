...
public class Formatter extends AbstractBehavior<Formatter.Message> {
    ...
    private Behavior<Formatter.Message> onMsg(Formatter.Message msg) {
        ...
        // As the two formatting doesnt depend on each other, the smallest expression
        // would be done first
        // And sent to continuation. Continuation can thus never make sure that what it
        // first gets is actually the
        // left expression.
        this.getContext().getSelf().tell(new Formatter.Message(cont, info.left, FormatterCont.LeftOrRight.Left));
        getContext().spawnAnonymous(Formatter.create())
                .tell(new Formatter.Message(cont, info.right, FormatterCont.LeftOrRight.Right));

        // this.getContext().getSelf().tell(new Formatter.Message(cont,info.right,
        // FormatterCont.LeftOrRight.Right));
